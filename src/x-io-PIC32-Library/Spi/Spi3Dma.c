...
#ifdef _SPI3_TX_IRQ
    DCH2ECONbits.CHSIRQ = _SPI3_TX_IRQ; // channel transfer start IRQ
#else
    DCH2ECONbits.CHSIRQ = _SPI3_TX_VECTOR; // channel transfer start IRQ
#endif
...
#ifdef _SPI3_RX_IRQ
    DCH3ECONbits.CHSIRQ = _SPI3_RX_IRQ; // channel transfer start IRQ
#else
    DCH3ECONbits.CHSIRQ = _SPI3_RX_VECTOR; // channel transfer start IRQ
#endif
...
void Dma3InterruptHandler(void) {
    EVIC_SourceStatusClear(INT_SOURCE_DMA3); // clear interrupt flag first because transfer complete callback may start a new transfer
    if (csPin != GPIO_PIN_NONE) {
#ifdef SPI3_CS_ACTIVE_HIGH
        GPIO_PinClear(csPin);
#else
...
bool Spi3DmaTransferInProgress(void) {
    return DCH3CONbits.CHEN == 1; // if RX DMA channel enabled
}

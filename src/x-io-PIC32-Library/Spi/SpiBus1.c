...
static SpiBusClient clients[SPI_BUS_1_MAX_NUMBER_OF_CLIENTS];
...
static void BeginTrasnfer(SpiBusClient * const client) {
    ...
    if (activeClient == NULL) {
        activeClient = client;
        SPI_BUS_1_TRANSFER(activeClient->csPin, activeClient->data, activeClient->numberOfBytes, TransferComplete);
    }

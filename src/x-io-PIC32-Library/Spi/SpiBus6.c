...
static Client clients[SPI_BUS_6_MAX_NUMBER_OF_CLIENTS];
...
static void BeginTrasnfer(Client * const client) {
    ...
    if (activeClient == NULL) {
        activeClient = client;
        SPI_BUS_6_TRANSFER(activeClient->csPin, activeClient->data, activeClient->numberOfBytes, TransferComplete);
    }

...
static Client clients[SPI_BUS_2_MAX_NUMBER_OF_CLIENTS];
 ...
 * @return Client.
 */
SpiBusClient SpiBus2AddClient(const GPIO_PIN csPin) {
    if (numberOfClients >= SPI_BUS_2_MAX_NUMBER_OF_CLIENTS) {
        return NULL;
    }
 ...
 */
void SpiBus2Transfer(const SpiBusClient client, void* const data, const size_t numberOfBytes, void (*transferComplete)(void)) {
    if (client == NULL) {
        return;
    }
 ...
 */
bool SpiBus2TransferInProgress(const SpiBusClient client) {
    if (client == NULL) {
        return false;
    }
...
static void BeginTrasnfer(Client * const client) {
    ...
    if (activeClient == NULL) {
        activeClient = client;
        SPI_BUS_2_TRANSFER(activeClient->csPin, activeClient->data, activeClient->numberOfBytes, TransferComplete);
    }

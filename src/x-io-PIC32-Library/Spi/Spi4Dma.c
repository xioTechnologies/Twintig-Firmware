 ...
 */
void Spi4DmaTransfer(const GPIO_PIN csPin_, volatile void* const data_, const size_t numberOfBytes_, void (*transferComplete_)(void)) {
...
void Dma5InterruptHandler(void) {
    EVIC_SourceStatusClear(INT_SOURCE_DMA5); // clear interrupt flag first because transfer complete callback may start new transfer
    if (csPin != GPIO_PIN_NONE) {
#ifdef SPI4_CS_ACTIVE_HIGH
        GPIO_PinClear(csPin);
#else

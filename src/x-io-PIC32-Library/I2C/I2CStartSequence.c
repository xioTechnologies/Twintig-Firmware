 ...
 */
bool I2CStartSequence(void (*i2cStart)(void), bool(*i2cSend)(const uint8_t byte), const uint8_t address, const uint32_t timeout_) {
    const uint64_t timeout = TimerGetTicks64() + ((uint64_t) timeout_ * (uint64_t) TIMER_TICKS_PER_MILLISECOND);
    while (true) {
        i2cStart();
        if (i2cSend(I2CAddressWrite(address))) {
            return true;
        }

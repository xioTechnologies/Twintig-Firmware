...
static uint8_t fifoData[ICM16_BUFFER_LENGTH * sizeof (IcmFifoPacket)];
...
void Icm16Initialise(const IcmOdr odr) {
    ...
    if (spiBusClient == NULL) {
        spiBusClient = ICM16_SPI_BUS_ADD_CLIENT(ICM16_CS_PIN);
    }
...
void Icm16Deinitialise(void) {
    ...
    while (ICM16_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
    FifoClear(&fifo);
    bufferOverflow = 0;
}
...
static uint8_t ReadRegister(const uint8_t address) {
    ...
    ICM16_SPI_BUS_TRANSFER(spiBusClient, &spiPacket, 2, NULL);
    while (ICM16_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
...
static void ExternalInterrupt(GPIO_PIN pin, uintptr_t context) {
    ...
    if (ICM16_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient)) {
        return;
    }
    timestamp = TimerGetTicks64();
    spiPacket.rw = 1;
    spiPacket.address = ICM_TEMP_DATA1_ADDRESS;
    ICM16_SPI_BUS_TRANSFER(spiBusClient, (void *const) &spiPacket, sizeof (IcmSensorRegisters) + 1, TransferComplete);
}

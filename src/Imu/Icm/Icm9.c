...
void Icm9Initialise(const IcmOdr odr) {
    ...
    if (spiBusClient == NULL) {
        spiBusClient = ICM9_SPI_BUS_ADD_CLIENT(ICM9_CS_PIN);
    }
...
void Icm9Deinitialise(void) {
    ...
    while (ICM9_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
    FifoClear(&fifo);
    bufferOverflow = 0;
}
...
static uint8_t ReadRegister(const uint8_t address) {
    ...
    ICM9_SPI_BUS_TRANSFER(spiBusClient, &spiPacket, 2, NULL);
    while (ICM9_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
...
static void ExternalInterrupt(GPIO_PIN pin, uintptr_t context) {
    ...
    if (ICM9_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient)) {
        return;
    }
    timestamp = TimerGetTicks64();
    spiPacket.rw = 1;
    spiPacket.address = ICM_TEMP_DATA1_ADDRESS;
    ICM9_SPI_BUS_TRANSFER(spiBusClient, (void *const) &spiPacket, sizeof (IcmSensorRegisters) + 1, TransferComplete);
}

...
void Icm18Initialise(const IcmOdr odr) {
    ...
    // Add client
    if (client == NULL) {
        client = ICM18_SPI_BUS_ADD_CLIENT(ICM18_CS_PIN);
    }
...
void Icm18Deinitialise(void) {
    ...
    while (ICM18_SPI_BUS_TRANSFER_IN_PROGRESS(client));
    FifoClear(&fifo);
    bufferOverflow = 0;
}
...
static uint8_t ReadRegister(const uint8_t address) {
    ...
    ICM18_SPI_BUS_TRANSFER(client, &spiPacket, 2, NULL);
    while (ICM18_SPI_BUS_TRANSFER_IN_PROGRESS(client));
...
static void ExternalInterrupt(GPIO_PIN pin, uintptr_t context) {
    ...
    if (ICM18_SPI_BUS_TRANSFER_IN_PROGRESS(client)) {
        return;
    }
    timestamp = TimerGetTicks64();
    spiPacket.rw = 1;
    spiPacket.address = ICM_TEMP_DATA1_ADDRESS;
    ICM18_SPI_BUS_TRANSFER(client, (void *const) &spiPacket, sizeof (IcmSensorRegisters) + 1, TransferComplete);
}

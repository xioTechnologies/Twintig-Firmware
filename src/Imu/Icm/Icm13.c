...
static uint8_t fifoData[ICM13_BUFFER_LENGTH * sizeof (IcmFifoPacket)];
...
void Icm13Initialise(const IcmOdr odr) {
    ...
    if (spiBusClient == NULL) {
        spiBusClient = ICM13_SPI_BUS_ADD_CLIENT(ICM13_CS_PIN);
    }
...
void Icm13Deinitialise(void) {
    ...
    while (ICM13_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
    FifoClear(&fifo);
    bufferOverflow = 0;
}
...
static uint8_t ReadRegister(const uint8_t address) {
    ...
    ICM13_SPI_BUS_TRANSFER(spiBusClient, &spiPacket, 2, NULL);
    while (ICM13_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
...
static void ExternalInterrupt(GPIO_PIN pin, uintptr_t context) {
    ...
    if (ICM13_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient)) {
        return;
    }
    timestamp = TimerGetTicks64();
    spiPacket.rw = 1;
    spiPacket.address = ICM_TEMP_DATA1_ADDRESS;
    ICM13_SPI_BUS_TRANSFER(spiBusClient, (void *const) &spiPacket, sizeof (IcmSensorRegisters) + 1, TransferComplete);
}

...
void Icm1Initialise(const IcmOdr odr) {
    ...
    // Configure interrupt
    GPIO_PinInterruptCallbackRegister(INT4_CH1_PIN, ExternalInterrupt, 0);
...
static void ExternalInterrupt(GPIO_PIN pin, uintptr_t context) {
    ...
    Spi4DmaTransfer(CS4_CH1_PIN, (void *const) &spiPacket, sizeof (IcmSensorRegisters) + 1, TransferComplete);
}

...
static uint8_t fifoData[ICM15_BUFFER_LENGTH * sizeof (IcmFifoPacket)];
 ...
 * @param odr ODR.
 */
void Icm15Initialise(const IcmOdr odr) {

    // Add SPI bus client
    if (spiBusClient == NULL) {
        spiBusClient = ICM15_SPI_BUS_ADD_CLIENT(ICM15_CS_PIN);
    }
...
void Icm15Deinitialise(void) {
    ...
    while (ICM15_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
    FifoClear(&fifo);
    bufferOverflow = 0;
}
...
static uint8_t ReadRegister(const uint8_t address) {
    ...
    ICM15_SPI_BUS_TRANSFER(spiBusClient, &spiPacket, 2, NULL);
    while (ICM15_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient));
...
static void ExternalInterrupt(GPIO_PIN pin, uintptr_t context) {
    ...
    if (ICM15_SPI_BUS_TRANSFER_IN_PROGRESS(spiBusClient)) {
        return;
    }
    timestamp = TimerGetTicks64();
    spiPacket.rw = 1;
    spiPacket.address = ICM_TEMP_DATA1_ADDRESS;
    ICM15_SPI_BUS_TRANSFER(spiBusClient, (void *const) &spiPacket, sizeof (IcmSensorRegisters) + 1, TransferComplete);
}

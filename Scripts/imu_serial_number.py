...
try:
    ...
    imu_connections = ximu3.helpers.mux_connect(carpus_connection, 20)

    unix_timestamp = int(time.time())

    for ch_index in range(5):
        serial_number = ximu3.helpers.send_command(imu_connections[4 * ch_index], "serial_number")
        ...
        for imu_index in range(4):
            ...
            ximu3.helpers.send_command(connection, "serial_number", f"{(unix_timestamp + ch_index):08X}-{imu_index + 1}")
            ximu3.helpers.send_command(connection, "hardware_version", "v1.0.4")  # edit this value as required
